...
//        }
        ...
        public function allMyMessages(Request $request, $roomId)
        {
            ...
            $adminMessages = ChatMessage::select('user_id')->distinct()->where(['admin_id' => $admin->id, 'chat_room_id' => $roomId])->with('user')->get();
//            $adminMessages = ChatMessage::select('user_id')->distinct()->where(['admin_id' => $admin->id, 'chat_room_id' => $roomId])->orderBy('created_at', 'DESC')->get();
